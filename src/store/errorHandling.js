...
const errorHandling = new Vue({
    ...
    methods:{
        openNotification(options) {
          ...
          errLog(msg) {
            if (msg.response.status >= 400 && msg.response.status < 500 ) {
        ...
        toaster(title, type, values,duration){
            if(title == ""){
                ...
                //     duration: duration,
                // });
        store.commit('SET_SNAKBAR', { msg: values, show: true, color: 'red lighten-1', timeout: duration }, { root: true })
            }else{
                // this.openNotification({
                //     title: title,
                //     type: type,
                ...
                //     duration: duration,
                // });
                store.commit('SET_SNAKBAR', { msg: title, show: true, color: 'red lighten-1', timeout: duration }, { root: true })
            }
